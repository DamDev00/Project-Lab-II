...
- ogni file.c ha ovviamente un relativo file.h in cui inizialmente definisco delle macro con il nome del file in grassetto per non avere problemi di ridefinizioni multiple (ovviamente non a tutti i file.h)
...
- Per ogni problema con l'allocazione della memoria, apertura di un file, o per errore di sintassi, viene esplicitato il tipo di errore riportando il nome del file e la linea di codice.

- Le uniche chiamate di sistema che ho utilizzato per le operazioni nel file.log sono protette da MACRO.
  
- La memoria viene liberata in un colpo solo nel server.c, dato che tutti i riferimenti passeranno ad essa.

- Per compilare tutto, digitare 'make' per poi avviare il client e il server a parte, oppure digitare 'make run' compilando tutto e avviando subito il server.

- segnalo che ho utilizzato due funzioni che non abbiamo visto a lezione, tra cui: int snprintf(char *str, size_t size, const char *format, ...); per sovrascrivere un buffer con una stringa formattata, semplificando molte righe di codice; struct tm *<var> = localtime(&<current time>); di cui ne parlerà più avanti.

## Come ho gestito le emergenze

- Nella griglia può essere gestita più di un'emergenza nella stessa posizione (x;y);
- Gli unici motivi per cui può essere scartata un'emergenza:
  1) non c'è nemmeno un istanza di un tipo di soccorritore necessario per l'emergenza;
  2) uno o più soccorritori non riescono ad arrivare in tempo al punto d'emergenza per la priorità
- Se per un emergenza mancano una o più istanze di un soccorritore necessario, allora, per i gemelli digitali di quel tipo,
  gli verrà assegnato del tempo in più per ricoprire il tempo che non possono impiegare le istanze mancanti.
  ESEMPIO: ho [incendio][...]Pompieri:2,4;Polizia:3,5; nel file **emergency.conf** e [Pompieri][2][...][...], [Polizia][1][...][...] nel file **rescuers.conf**.
  In questo caso ci sono tutti i pompieri, quindi ogni pompiere gestisce l'emergenza per 4 secondi. Per quanto riguarda la polizia mancano due istanze, perciò quell'unica istanza
  deve ricoprire il lavoro degli altri poliziotti, dato che il tempo totale è dato da <istanze richieste poliziotti> * <tempo dedicato> = 3 * 5 = 15, verrano assegnati, ad ogni
  gemello digitale di tipo poliziotto (<istanze richieste poliziotti> * <tempo dedicato>) / (istanze disponibili al momento) = 15 / 1 = 15. Quindi quel poliziotto dovrà essere
  lì presente per 15 secondi.
- Se per un emergenza ci sono 0 istanze di un tipo di soccorritore disponibili, non per preimpostazione del file **rescuers.conf**, ma perchè sono occupati per un'altra emergenza,
  allora viene inserita nella coda d'attesa.
- La coda d'attesa contiene le emergenze in ordine descrescente, per priorità, partendo dall'indice 0.
- Ogni tipo di emergenza che riceve il server deve avere: un nome presente nel file **emergency.conf** e le coordinate che rientrano nelle dimensioni della griglia. 
- Più avanti verranno approfondite le implementazioni del server che riguarderà principalmente la gestione delle emergenze.
  
## Per ciascun parser

- Inizializzo l'id, l'event, e un array di caratteri all'inizio che servono per la scrittura nel file .log;
...
# functions.c
 - Contiene 4 funzioni tra cui:
   ...
   - utilizzato per aggiornare il file.log
   ...
   ## int digits(int x)
   - semplice funzione che utilizzo nel client per contare il numero di cifre di un numero
...
## Funzioni
- int send_queue(char* filename,  emergency_request_t* msg): permette di inviare l'emergenza al server. **filename** è il nome del file **parse_env.c** per il nome della message queue, mentre **msg** racchiude il nome e le coordinate dell'emergenza.
- void stop_message_queue(): permette di far terminare la message queue del server.
- int send_with_parameters(char** parameters): permette di settare l'emergenza da inviare tramite i parametri da terminale.
- int send_with_file(char* file_emergency): permette di inviare le emergenze da un file.{conf,text}.

## Istruzioni per l'invio delle emergenze
...
- Se si vuole interrompere la message queue da terminare, digitare ./<nome file client> STOP
