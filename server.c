...
int handler_waiting_queue(void* args){
    ...
    */
    ...
    if(thrd_create(&control_waiting_queue_t, control_waiting_queue, NULL) == thrd_success){
        ...
        printf("[ERRORE CON LA DETACH: %s;%d]\n", __FILE__,__LINE__);
        exit(0);
    }
...
int start_emergency(emergency_id_t* current_emergency){
    ...
    */
    ...
    /*
        la flag in_loading viene controllata nella funzione che gestisce la 
        coda d'attesa. In sostanza, può capitare che la funzione handler_waiting_queue
        riprenda l'indice di un'emergenza già avviata dopo essere stata prelevata dalla message
        queue. Fino a quando non viene eliminata, potrebbe capitare, ma non viene
        riavviata due volte grazie a questa flag
    */
        ...
        */
        ...
        if(num_request == 0){
            ...
            /*
                ...
                essere soddisfatta per la completa mancanza di tipo un soccorritore (0 istanze).
                quindi inserisco l'emergenza nella coda d'attesa
            */

            // aggiorno la flag in_loading
        ...
        */
        ...
        for(int j = 0; tmp->avalaible_resc > j; j++){
            ...
            */
            ...
            /*
                in particolare è fondalmentale settare l'id dell'emergenza corrente che deve gestire
                per poter rintracciare qualche sua informazione nell'array globale delle emergenze
            */
...
int handler_emergency(void* args){
    ...
    */
    ...
    /*
        ...
        necessari per l'emergenza corrente. 
        Può restituire -1, 0, e 1 a seconda delle situazioni:
        - (-1) se non esiste nessuna istanza per un determinato tipo di
